...
static int attiny_i2c_read(struct i2c_client *client, u8 reg, unsigned int *buf)
{
	...
	u8 data_buf[1] = { 0, };
	int ret;
	printk(KERN_ERR"clien->addr=%x......",client->addr);
	...
	ret = i2c_transfer(client->adapter, msgs, ARRAY_SIZE(msgs));
	if (ret != ARRAY_SIZE(msgs)){
		dev_err(&client->dev, " client->dev  Failed to read REG_ID reg: %d \n", ret);
		...
		const struct i2c_device_id *id)
{
	...
	unsigned int data;
	int ret;
	printk(KERN_ERR"attiny_i2c_probe........");
	...
	if (ret < 0) {
		dev_err(&i2c->dev, "Failed to read REG_ID reg: %d ,client addr:%x,read data: %x\n", ret,i2c->addr,data);

		goto error;
	}
	dev_err(&i2c->dev, "Corrected to read REG_ID reg: %d ,client addr:%x,read data: %x\n", ret,i2c->addr,data);
...
static void attiny_i2c_remove(struct i2c_client *client)
{
	...
	printk(KERN_ERR"attiny_i2c_remove........");
	mutex_destroy(&state->lock);
}
