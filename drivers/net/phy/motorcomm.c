...
static int yt8531_config_init(struct phy_device *phydev)
{
	...
	ret = ytphy_config_init(phydev);
#else
	printk ("llj..get YT8511, abt to set 125m clk out, phyaddr=%d, phyid=%08x\n",phydev->mdio.addr, phydev->phy_id);
	rett = yt8511_config_out_125m(phydev->mdio.bus, phydev->mdio.addr);
	printk ("llj..8511 set 125m clk out, reg=%#04x\n",phydev->mdio.bus->read(phydev->mdio.bus,phydev->mdio.addr,0x1f)/*double check as delay*/);
	if (rett<0)
			printk ("llj..failed to set 125m clk out, rett=%d\n",rett);
	ret = genphy_config_init(phydev);
#endif
	if (ret < 0)
		return ret;
